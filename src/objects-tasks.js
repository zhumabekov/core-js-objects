...
function mergeObjects(objects) {
  ...
  for (let index = 1; index < objects.length; index += 1) {
    Object.entries(objects[index]).forEach(([key, val]) => {
      if (key in result) {
...
function fromJSON(proto, json) {
  ...
  Object.entries(parsedObject).forEach(([key, val]) => {
    objectWithProto[key] = val;
  });
...
function group(array, keySelector, valueSelector) {
  const a = new Map();
  array.forEach((item) => {
    if (a.has(keySelector(item))) {
      a.set(keySelector(item), [
        ...a.get(keySelector(item)),
        valueSelector(item),
      ]);
    } else {
      a.set(keySelector(item), [valueSelector(item)]);
    }
  });
  return a;
}
